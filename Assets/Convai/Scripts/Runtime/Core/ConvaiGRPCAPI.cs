...
#else
    ConvaiLogger.Warn("StartRecordAudio is not supported on WebGL platform.", ConvaiLogger.LogCategory.Character);
#endif
}
    ...
    while (Microphone.IsRecording(MicrophoneManager.Instance.SelectedMicrophoneName))
    {
        ...
        if (diff > 0)
        {
            if (audioClip == null)
            {
                ...
                _cancellationTokenSource?.Dispose();
                _cancellationTokenSource = null;
                ConvaiLogger.Info("Audio clip is null. Disposed token source.", ConvaiLogger.LogCategory.Character);
                break;
            }
